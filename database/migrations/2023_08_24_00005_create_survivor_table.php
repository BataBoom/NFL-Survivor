...
class CreateSurvivorTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('survivor', function (Blueprint $table) {
            ...
            $table->foreignId('uid')->references('id')->on('users')->onDelete('cascade')->cascadeOnDelete();
            $table->foreignId('selection_id')->references('team_id')->on('wager_options')->cascadeOnDelete();
            $table->foreignId('pool')->references('id')->on('pools')->cascadeOnDelete();
            $table->integer('week');
            $table->string('selection');
            $table->boolean('result')->nullable();
            $table->timestamps();
            $table->unique(['selection_id', 'selection', 'uid', 'pool', 'week']);
